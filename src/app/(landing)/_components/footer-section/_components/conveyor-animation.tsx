...
import { CARD_FACES, cardFaceAlt, resolveCardFaceSrc } from "@/lib/card-faces";
...
export function ConveyorAnimation() {
  ...
  useEffect(() => {
    const shuffled = [...CARD_FACES].sort(() => 0.5 - Math.random());
  ...
  const getBoxIcon = (index: number) => {
    const face = randomFaces[index] || CARD_FACES[index % CARD_FACES.length];
    return (
      <img
        src={resolveCardFaceSrc(face)}
