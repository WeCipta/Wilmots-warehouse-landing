...
import { CARD_FACES, cardFaceAlt } from "@/lib/card-faces";
...
function ColorfulLink({ label, href }: { label: string; href: string }) {
  ...
  return (
    <a
      ...
      className="text-3xl md:text-5xl font-black tracking-tight text-black/70 hover:-translate-y-1 hover:-rotate-2 transition-transform duration-300 w-fit inline-block origin-left"
      style={{
...
import { ConveyorAnimation } from "./_components/conveyor-animation";
...
export function FooterSection() {
  ...
  useEffect(() => {
    ...
    const wallLeft = Matter.Bodies.rectangle(-250, -5000, 500, 15000, {
      isStatic: true,
    });
    const wallRight = Matter.Bodies.rectangle(width + 250, -5000, 500, 15000, {
      isStatic: true,
    });
      ...
      <div className="bg-[#FFFDEB] text-black w-full h-[100svh] xl:h-full px-8 pb-8 pt-28 md:px-6 md:pb-8 md:pt-40 flex flex-col justify-between relative z-10 pointer-events-auto border-b xl:border-b-0 xl:border-r border-black/10 overflow-hidden">
        ...
        <div className="flex flex-col gap-32 mt-12 relative z-10">
          <Button
            variant="filled"
            size="lg"
            className="h-11 px-5 text-base sm:h-14 sm:px-8 sm:text-2xl w-fit group bg-black text-white"
            nativeButton={false}
            render={
              <a
          ...
          <div className="text-xs md:text-sm font-bold text-black/40 uppercase tracking-[0.2em] flex flex-col gap-2">
            ...
            <p>
              ...
              >
                {siteContent.credits.publisher.name}
              </a>
            </p>
          </div>
        </div>
